...
9. **This session's 66 new tests have not yet been observed passing in a real GitHub Actions run** — verified locally (90/90) and the CI workflow already discovers them with no changes needed, but the first actual scheduled/triggered run since they were added hadn't completed as of this doc update. Worth a one-time check that it goes green rather than assuming it will.
